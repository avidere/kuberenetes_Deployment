                ...
                script{
                    sshagent(['Docker-Server']) {
                         ...
                         // sh "ssh -o StrictHostKeyChecking=no -l dockeradmin 172.31.2.23 sudo mv Pythonapp-deployment /home/ubuntu/ "
                          sh "ssh -o StrictHostKeyChecking=no -l dockeradmin 172.31.2.23 sudo sed -i 's/tag/${env.BUILD_NUMBER}/g' /Pythonapp-deployment/web_deployment.yaml"
                          sh "ssh -o StrictHostKeyChecking=no -l dockeradmin 172.31.2.23 sudo cp /home/ubuntu/Pythonapp-deployment/*.yaml service.yaml /home/ubuntu/"
                          sh "ssh -o StrictHostKeyChecking=no -l dockeradmin 172.31.2.23 docker build -t avinashdere99/python:${env.build_no} /Pythonapp-deployment/Dockerfile"
                          sh "ssh -o StrictHostKeyChecking=no -l dockeradmin 172.31.2.23 docker login -u $docker_user -p $docker_pass"
                          sh "ssh -o StrictHostKeyChecking=no -l dockeradmin 172.31.2.23 sudo rm -r Pythonapp-deployment /home/ubuntu/ "
